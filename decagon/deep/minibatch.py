...
class EdgeMinibatchIterator(object):
    ...
    def __init__(self, adj_mats, feat, edge_types, nsz_fold, tra_f, tra_edges_t, tra_edges_f, test_edges_t,
                 test_edges_f, val_edges_t, val_edges_f,
    ...
    def batch_feed_dict(self, batch_edges, batch_neg, batch_pos_s, batch_neg_s, batch_edge_type, placeholders):
        ...
        feed_dict.update({placeholders[
                              'batch']: batch_edges})  # self.batch_feed_dict(batch_edges, self.current_edge_type_idx, placeholders)
    ...
    def next_minibatch_feed_dict(self, placeholders):
        ...
        if self.current_edge_type_idx > 3:
            self.batch_pos_s = np.array(
                [self.labels_for_reg[self.current_edge_type_idx - 4][tuple(i)] for i in self.batch_edges])
            self.batch_neg_s = np.array(
                [self.labels_for_reg[self.current_edge_type_idx - 4][tuple(i)] for i in self.batch_neg])
        else:
            self.batch_pos_s = np.array([1. for i in self.batch_edges])
            self.batch_neg_s = np.array([0. for i in self.batch_neg])
        return self.batch_feed_dict(self.batch_edges, self.batch_neg, self.batch_pos_s, self.batch_neg_s,
                                    self.current_edge_type_idx, placeholders)
