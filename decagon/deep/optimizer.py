...
class DecagonOptimizer(object):
    def __init__(self, embeddings, latent_inters, latent_varies,
                 degrees,LOSS,edge_types, edge_type2dim,neg_sample_weights,placeholders,
                 margin=0.1,  batch_size=100):
        ...
        self.inputs_S = placeholders['batch_pos_s']
        self.inputs_neg_S = placeholders['batch_neg_s']
    ...
    def _mse_loss(self,aff,neg_aff,):
        true_ = tf.losses.mean_squared_error(aff, self.inputs_S)
        negative_ = tf.losses.mean_squared_error(neg_aff, self.inputs_neg_S)
        loss = tf.reduce_sum(true_) + self.neg_sample_weights * tf.reduce_sum(negative_)
        return loss
...
def gather_cols(params, indices, name=None):
    ...
    with tf.name_scope( name, "gather_cols",[params, indices]) as scope:
        ...
        op=tf.reshape(tf.gather(p_flat, i_flat), [p_shape[0], -1])
        return op
