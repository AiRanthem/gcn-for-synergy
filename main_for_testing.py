...
tra_edges_t,tra_edges_f=get_set(Model_Parameters['Sep_set'][0]+Model_Parameters['Sep_set'][1],Model_Parameters['threshold'])
...
for raw_epoch in range(FLAGS.Max_epoch):
    ...
    while not minibatch.end():
        ...
        if itr % PRINT_PROGRESS_EVERY == 0:
            print("Edge:", "%-3s%-14s  tra_loss=%f " % (batch_edge_type,idx2name[minibatch.current_edge_type_idx],train_cost))
        itr += 1
...
for et in range(num_edge_types):
    ...
    try:
        ...
        output_accuracy_scores[str(minibatch.idx2edge_type[et])]=[edge_Type,roc_score,auprc_score,test_mse,test_pearson,\
